    ...
    class Mkdir
    {
        ...
        public static void c_Mkdir(string mkdir, short startIndex = 0, short count = 6)
        {
            ...
            else
            {
                ...
                {
                    Directory.CreateDirectory(Uszka.Kernel.cd + dir);
                }
                else if (Directory.Exists(Uszka.Kernel.cd + dir))
                {
                    Char[] separators = new char[] { '(', ')' };
                    Char[] directories = new char[] { '/', '\\' };

                    //getting last directory.
                    string x1 = Uszka.Kernel.cd + dir;
                    ...
                    if ((DirectoryAlreadyExist.Contains("(")) && (DirectoryAlreadyExist.Contains(")")))
                    {
                        ...
                        if (Directory.Exists(Uszka.Kernel.cd + newName))
                        {
                            goto boucle;
                        }

                        //create directory
                        Directory.CreateDirectory(Uszka.Kernel.cd + newName);
                    ...
                    else
                    {
                        ...
                        Directory.CreateDirectory(Uszka.Kernel.cd + dir + "(1)");
