    ...
    class CP
    {
        ...
        public static void c_CP(string cp, short startIndex = 0, short count = 3)
        {
            ...
            if (!cmdargs[1].StartsWith("-")) //WITHOUT ARGS, NO OVERWRITING
            {
                ...
                if (cmdargs.Length == 3)
                {
                    if (File.Exists(Uszka.Kernel.cd + sourcefile))
                    {
                        if (!File.Exists(Uszka.Kernel.cd + destfile))
                        {
                            try
                            {
                                File.Copy(Uszka.Kernel.cd + sourcefile, Uszka.Kernel.cd + destfile);
                    ...
                    if (cmdargs.Length == 4)
                    {
                        if (File.Exists(Uszka.Kernel.cd + sourcefile))
                        {
                            if (File.Exists(Uszka.Kernel.cd + destfile))
                            {
                                try
                                {
                                    File.Copy(Uszka.Kernel.cd + sourcefile, Uszka.Kernel.cd + destfile, true);
